...
LegoResult LegoFile::Read(void* p_buffer, LegoU32 p_size)
{
	...
	if (SDL_RWread(m_file, p_buffer, 1, p_size) != p_size) {
		return FAILURE;
	}
	return SUCCESS;
}
...
LegoResult LegoFile::Write(const void* p_buffer, LegoU32 p_size)
{
	...
	if (SDL_RWwrite(m_file, p_buffer, 1, p_size) != p_size) {
		return FAILURE;
	}
	return SUCCESS;
}
...
LegoResult LegoFile::GetPosition(LegoU32& p_position)
{
	...
	Sint64 position = SDL_RWtell(m_file);
	if (position == -1) {
		return FAILURE;
	}
	p_position = position;
...
LegoResult LegoFile::SetPosition(LegoU32 p_position)
{
	...
	if (SDL_RWseek(m_file, p_position, RW_SEEK_SET) != p_position) {
		return FAILURE;
	}
	return SUCCESS;
}
...
LegoResult LegoFile::Open(const char* p_name, LegoU32 p_mode)
{
	...
	if (!(m_file = SDL_RWFromFile(path.GetData(), mode))) {
		return FAILURE;
	}
	return SUCCESS;
}
